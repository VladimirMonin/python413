...
# 4. Interface Segregation Principle (Принцип разделения интерфейса)
# много специализированных интерфейсов лучше, чем один универсальный. Как в примере ниже, если бы я записал анализ блокчейна в PaymentProcessor, то это было бы нарушение принципа. Потому что не все платежные системы используют блокчейн, но пришлось бы реализовать метод во всех классах.
...
# Представь, что Open/Closed принцип — это когда ты делаешь модульный шкаф, к которому легко добавлять новые секции, не разбирая уже собранное. Ты просто пристраиваешь новые модули сбоку. А Dependency Inversion — это когда ты изначально проектируешь шкаф так, чтобы все его компоненты соединялись через универсальные крепления, и ты мог поменять дверцы от одного производителя на другие без переделки всего шкафа.

# В коде они действительно часто идут рука об руку! Open/Closed говорит: "Не трожь работающий код, лучше расширь его". А Dependency Inversion подсказывает: "Используй абстракции, чтобы высокоуровневые модули не зависели от низкоуровневых деталей". Оба принципа активно используют абстракции и полиморфизм.
