...
VALUES
    (
        ...
        'Первое занятие по SQLite',
        2
    );

-- Пример домашнего задания
INSERT INTO
    homeworks (online_lesson_id, summary, homework_text)
VALUES
    (
        1,
        'Создание первой базы данных',
        'Создайте базу данных для библиотеки с таблицами books и authors'
    );
...
-- =============================================================================
/*
-- Получить всех студентов группы с их статистикой
SELECT * FROM v_attendance_stats WHERE group_name = 'Python Разработчики 2025';

-- Найти студентов с низкой посещаемостью
SELECT * FROM v_attendance_stats WHERE avg_mark < 6;

-- Получить просроченные домашки
SELECT 
s.first_name || ' ' || s.last_name AS student_name,
h.summary,
h.deadline_date,
hs.status
FROM homeworks h
JOIN homeworks_students hs ON h.id = hs.homework_id
JOIN students s ON hs.student_id = s.id
WHERE h.deadline_date < DATE('now') 
AND hs.status NOT IN ('принято', 'проверено', 'обратная связь выдана');

-- Статистика по группам
SELECT 
g.group_name,
COUNT(s.id) AS students_count,
COUNT(ol.id) AS lessons_count,
COUNT(h.id) AS homeworks_count
FROM groups g
LEFT JOIN students s ON g.id = s.group_id
LEFT JOIN online_lessons ol ON g.id = ol.group_id
LEFT JOIN homeworks h ON ol.id = h.online_lesson_id
